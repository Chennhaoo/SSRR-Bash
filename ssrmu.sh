...
View_User_info(){
	...
	echo -e " 加密\t    : ${Red_font_prefix}${method}${Font_color_suffix}"
...
Set_config_port(){
	...
	stty erase '^H' && read -p "(默认: 9300):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="9300"
...
Set_config_method(){
 ...
 ${Green_font_prefix}11.${Red_font_prefix} salsa20
 ${Green_font_prefix}12.${Red_font_prefix} chacha20
 ${Green_font_prefix}13.${Red_font_prefix} chacha20-ietf
 
 ${Green_font_prefix}14.${Red_font_prefix} xsalsa20
 ${Green_font_prefix}15.${Red_font_prefix} xchacha20
 ${Tip} salsa20/chacha20-* 系列加密方式，需要额外安装依赖 libsodium ，否则会无法启动ShadowsocksRR !" && echo
	stty erase '^H' && read -p "(默认: 15. xchacha20 需安装libsodium):" ssr_method
...
————————————
 ${Green_font_prefix}10.${Font_color_suffix} 启动 ShadowsocksRR
 ${Green_font_prefix}11.${Font_color_suffix} 停止 ShadowsocksRR
